...
pub mod pyth_lazer_solana_contract {
    ...
    /// signature). The clean twin derives it internally in
    /// `signature::verify_message` and enforces a byte-for-byte
    /// `slice_eq` check. This planted dispatcher threads the
    /// caller-supplied `message_offset` through and the callee skips
    /// that check - the seeded specification violation.
