...
// This file is a planted-twin sibling of
// `../clean/pyth-lazer-solana-contract/src/signature.rs`. All hunks below
// that are NOT annotated with `planted:` are byte-identical to the
// clean (post-fix) source at pin
// pyth-network/pyth-crosschain@eb7f460ab8d1c73c6c8b4942891c9fe74a589121.
//
// The seeded specification violation: `verify_message` re-accepts a
// caller-supplied `message_offset: u16` argument (pre-eb7f460 signature)
// and skips the byte-for-byte `slice_eq(expected_message_data,
// message_data)` check between the ed25519-instruction-visible message
// bytes and the caller-supplied `message_data` argument. This is a
// mechanical reversal of the security-critical hunk of PR #2250.
//
// Everything else in this file - module imports, error enum, helper
// signatures, offset validation blocks, `slice_eq` definition (kept but
// no longer called), pubkey extraction, trusted-signer lookup, payload
// slicing - is byte-identical to the clean sibling.
...
// planted: `slice_eq` retained but unused in `verify_message` below.
// Kept so twin-diff surface stays limited to the security-critical hunk
// and grep-based provenance checks continue to see the helper symbol.
#[allow(dead_code)]
...
///
/// planted: `message_offset` is a caller-supplied argument (pre-eb7f460
/// signature). The clean twin derives it internally from
/// `offsets.signature_offset.checked_sub(MAGIC_LEN)` and enforces a
/// byte-for-byte `slice_eq(expected_message_data, message_data)` check
/// against the current instruction's data. This planted twin skips that
/// check - the seeded specification violation.
pub fn verify_message(
    ...
    // pre-fix behavior - check the caller-supplied `message_offset`
    // against `offsets.signature_offset`, and DO NOT enforce the
    // byte-for-byte `slice_eq(expected_message_data, message_data)`
    // check between the sysvar-visible instruction bytes and the
    // caller-supplied `message_data`. This decouples the identity the
    // helper attributes a message to from the identity the ed25519
